...
const Chat = () => {
  ...
  const allChatUserUpdate = () => {
    if (cookies?.user_data?.role === "professional") {
      cookies?.user_data && axios.post("http://13.52.16.160:8082/chat/all_chat_user/", {
      ...
      cookies?.user_data &&
        axios.post("http://13.52.16.160:8082/chat/all_chat_user/", {
          ...
          if (res?.data?.status === "Success") {
            ...
            axios.post("http://13.52.16.160:8082/chat/all_chats/", {
              professional_id: cookies?.user_data?.user_id,
              user_token: cookies?.user_data?.user_token,
              role: "professional",
              client_id:
            ...
            <Row className="chats-main" style={{ maxHeight: "100% " }}>
              <Col sm={3} id="chatusersdisplaybox" className="px-0" style={{
                ...
                {!searchState ? (
                  ...
                  <Nav variant="pills" className="flex-column">
                    {allChatUser && allChatUser?.filter((data) =>
                      data?.name?.toLowerCase()?.includes(searchPara.toLowerCase())).map((data, index) => (
                        <>
                          {data?.name ? (
                            <ChatItem
                              key={index}
              ...
              <Col
                ...
                {currentActiveChatPane !== "first" ? ("") : (
                  <Tab.Content>
                    <div id="chat-log" className={"tab-content d-flex flex-column w-100 pt-2"} style={{ position: " absolute", bottom: 0 }}>
                      <div className="chat-box">
                        {chatListData &&
                          chatListData.map((message, index) =>
                            message?.type === "text" ? (
                              <MessageBox key={index} title={message?.name} type={message?.type} text={message?.data} position={
                                message?.status === "Send" ? "right" : "left"
                              } />
                            ) : (
                              <a download target="_blank" href={message?.data}>
                                <MessageBox
                                  position={message?.status === "Send" ? "right" : "left"}
                                  type={message?.data.slice(-3) == "png" || "jpg" || "webm" ? "photo" : message?.type}
                                  title={message?.name}
                                  text={message?.data.slice(48)}
                                  copiableDate={true}
                                  data={{
                                    uri: message?.data, status: { click: false, loading: 0, },
                                  }}
                                />
                              </a>
                            )
                      ...
                      <form className="row mx-0 mb-2" onSubmit={sendFile ? fileSendContaroller : sendMessage} >
                        <div className="chatBoxFileSelectMainBox col-1">
                          <label className="fileSelectLinkChatbox" htmlFor="selectFile">
                            <AiOutlineLink color="white" style={{ cursor: 'pointer' }} />
                          </label>
                          <input type="file" name="file" id="selectFile" onChange={handleFileChange} />
                        </div>
                        <div className="col-11 d-flex input-area">
                          {sendFile && <div className="previewFile">
                            <button className="fileDeleteBtn btn btn-primery" onClick={handleDelete}>
                              <RxCross2 />
                            </button>
                            {
                              MsgType === "image" && <img src={filePreview} alt="" />
                            }
                            {
                              MsgType === "video" && <VideoPreview videoFile={sendFile} />
                            }{
                              MsgType === "application" && <div className="pdf-container">
                                <div style={{ fontSize: '200px', margin: 'auto' }}>
                                  <FaRegFilePdf />
                                </div>
                                <p>{sendFile.name}</p>
                              </div>
                          ...
                          <input className="chatMessageInputText" placeholder="Type here..." id="chat-message" value={messageInput} onChange={(e) => setMessageInput(e.target.value)} />
